...
std::shared_ptr<IRenderAPI> RenderAPIServiceLocator::mService = nullptr;

std::shared_ptr<IRenderAPI> Engine::CreateVulkanRenderBackEnd()
{
...
{
    std::cout << "Initializing Vulkan Render API" << std::endl;
...
void VulkanRenderAPI::CreateTexture(Texture& texture)
{
...
void VulkanRenderAPI::CreateBuffer(const BufferDesc& desc, const BufferData& data, std::shared_ptr<Buffer>& bufferObject)
{
...
void VulkanRenderAPI::CopyBuffer(std::shared_ptr<Buffer>& srcBuffer, std::shared_ptr<Buffer>& dstBuffer, const size_t srcOffset, const size_t dstOffset, const size_t size)
{
...
void VulkanRenderAPI::Deinitialize()
{
    ...
    std::cout << "Deinitialized Vulkan Render API" << std::endl;
}

void VulkanRenderAPI::TransitionImageLayout(VkImage& image, VkImageLayout oldLayout, VkImageLayout newLayout)
{
...
void VulkanRenderAPI::CopyBufferToImage(const VkBuffer& buf, const VkImage & image, const uint32_t width, const uint32_t height)
{
    OneTimeCommandListVK cmdBuffer(mCmdPool, mGraphicsQueue);
...
VkImageView VulkanRenderAPI::CreateImageView(const VkImage& image, VkFormat format, VkImageAspectFlagBits aspect)
{
...
VkSampler VulkanRenderAPI::CreateTextureSampler()
{
...
void VulkanRenderAPI::CreatePipeline(Effect& effect)
{
